...
/**
 * Shared shading. Every drawn icon renders these defs, so a flat fill can be
 * swapped for `url(#gloss)` and read as a rounded, lit object.
 */
function Shading() {
  return (
    <defs>
      <linearGradient id="gloss" x1="0" y1="0" x2="1" y2="1">
        ...
        <stop offset="1" stopColor="#4A2C2A" stopOpacity="0.18" />
      </linearGradient>
      <radialGradient id="plate" cx="38%" cy="32%" r="75%">
        <stop offset="0" stopColor="#FFFFFF" />
        <stop offset="1" stopColor="#F3E7D6" />
      </radialGradient>
      <radialGradient id="glass" cx="35%" cy="25%" r="80%">
        <stop offset="0" stopColor="#FFFFFF" />
        <stop offset="1" stopColor="#BFE3F7" />
      </radialGradient>
    </defs>
  );
}
...
export function SoftServeArt() {
  ...
  return (
    <svg {...svgProps}>
      ...
      {coils.map(([cx, cy, r], index) => (
        <ellipse key={`s${index}`} cx={cx} cy={cy} rx={r} ry={r * 0.56} fill="url(#gloss)" />
      ))}
    </svg>
  );
}
...
export function ScoopsArt() {
  ...
  return (
    <svg {...svgProps}>
      ...
      <circle cx="21" cy="42" r="12.5" fill="url(#gloss)" />
      <circle cx="43" cy="42" r="12.5" fill="url(#gloss)" />
      <circle cx="32" cy="22" r="14" fill="url(#gloss)" />
...
const VESSEL_ART: Record<VesselId, React.ReactElement> = {
  ...
  sundae: (
    <>
      <path d="M14 16h36l-9 20 3 12H24l3-12z" fill="url(#glass)" stroke={OUTLINE} strokeWidth="2.5" strokeLinejoin="round" />
...
/** A soft gloss laid over each vessel's body so it reads as a lit object. */
const VESSEL_SHINE: Record<VesselId, React.ReactElement> = {
  cup: <path d="M15 20h34l-4 32H19z" fill="url(#gloss)" />,
  "waffle-cone": <path d="M17 18h30L32 58z" fill="url(#gloss)" />,
  "cake-cone": <path d="M19 18h26l-6 36H25z" fill="url(#gloss)" />,
  sundae: <path d="M14 16h36l-9 20 3 12H24l3-12z" fill="url(#gloss)" />,
  float: <path d="M19 12h26l-4 44H23z" fill="url(#gloss)" />,
  "waffle-bowl": <path d="M10 24h44c0 16-10 26-22 26S10 40 10 24z" fill="url(#gloss)" />,
  "paper-boat": <path d="M6 24h52c-2 14-10 22-26 22S8 38 6 24z" fill="url(#gloss)" />,
  "egg-carton": <rect x="5" y="24" width="54" height="22" rx="5" fill="url(#gloss)" />,
  frosty: <path d="M20 18h24l-4 34H24z" fill="url(#gloss)" />,
};
...
export function ToppingArt({ topping }: { topping: Topping }) {
  ...
  return (
    <svg {...svgProps}>
      <Shading />
      <circle cx="32" cy="34" r="21" fill="url(#plate)" stroke={OUTLINE} strokeWidth="2.5" />
      {kind === "sprinkle" &&
...
/** A gold coin: bevel ring, a star, and a highlight. No currency symbol. */
export function CoinArt({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} aria-hidden="true">
      ...
      <circle cx="12" cy="12" r="10.5" fill="#B9791A" />
      <circle cx="12" cy="11.3" r="10" fill="url(#coin-face)" />
      <circle cx="12" cy="11.3" r="7.2" fill="none" stroke="#B9791A" strokeWidth="1.1" opacity="0.7" />
      <path
        d="M12 6.6l1.5 3.1 3.4.5-2.45 2.4.6 3.4L12 14.4 8.95 16l.6-3.4L7.1 10.2l3.4-.5z"
        fill="#B9791A"
        opacity="0.85"
      />
      <ellipse cx="9" cy="6.8" rx="2.6" ry="1.3" fill="#FFFFFF" opacity="0.55" transform="rotate(-30 9 6.8)" />
    </svg>
  );
}

/** A potted flowering plant, for the corner of the counter. */
export function PlantArt() {
  return (
    <svg {...svgProps}>
      <path d="M22 44h20l-2 16H24z" fill="#FFFDF7" stroke={OUTLINE} strokeWidth="2.5" strokeLinejoin="round" />
      <path d="M20 40h24v5H20z" fill="#F3EADA" stroke={OUTLINE} strokeWidth="2.5" strokeLinejoin="round" />
      {[
        [32, 26, 0],
        [22, 30, -30],
        [42, 30, 30],
      ...
      {[
        ...
        <g key={index}>
          <circle cx={cx} cy={cy} r="3.6" fill="#FFFFFF" stroke={OUTLINE} strokeWidth="1.5" />
          <circle cx={cx} cy={cy} r="1.3" fill="#FFD25E" />
        </g>
      ))}
    </svg>
  );
...
/** The little A-frame chalkboard by the door. */
export function ChalkboardArt() {
  return (
    <svg {...svgProps}>
      <path d="M18 12h28l6 46H12z" fill="#D9A05B" stroke={OUTLINE} strokeWidth="2.5" strokeLinejoin="round" />
      <path d="M22 17h20l4 36H18z" fill="#2E2724" stroke={OUTLINE} strokeWidth="2" strokeLinejoin="round" />
      <path
        d="M32 44c-5-3-8-6-8-10 0-3 2-5 4.5-5 1.6 0 2.8.8 3.5 2 .7-1.2 1.9-2 3.5-2 2.5 0 4.5 2 4.5 5 0 4-3 7-8 10z"
        fill="none"
        stroke="#FFF6E8"
        strokeWidth="2.2"
        strokeLinejoin="round"
      />
      <path d="M24 26h16" stroke="#FFF6E8" strokeWidth="1.6" strokeLinecap="round" opacity="0.7" />
      <path d="M14 58l6-12M50 58l-6-12" stroke={OUTLINE} strokeWidth="2.5" strokeLinecap="round" />
    </svg>
  );
}
